...
| Metadata schema and event immutability | Shared metadata is the governance record and must be robust across agents/backends. | 部分实现。已有 schema/version/ID/path validation、head/commit/event FS 一致性检查、初始化失败回滚、service event authority、event `seq`、事件路径覆盖拒绝、commit accepted 事件失败不推进 head。还缺 backend create-if-absent 断言。 |
